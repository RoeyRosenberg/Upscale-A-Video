...
def apply_random_video_compression(imgs, codec, bitrate):
    ...
    height, width = (imgs.shape[2] // 2) * 2, (imgs.shape[3] // 2) * 2
    imgs = F.interpolate(imgs, (height, width), mode=mode)

    video = (imgs * 255).permute(0, 2, 3, 1).cpu().numpy().astype(np.uint8)
    ...
    outputs = torch.zeros(video.shape)
    with av.open(buf, 'r', 'mp4') as container:
        if container.streams.video:
            for i, frame in enumerate(container.decode(**{'video': 0})):
                outputs[i] = torch.from_numpy(frame.to_rgb().to_ndarray().astype(np.float32)) / 255

    outputs = outputs.to(imgs.device).permute(0, 3, 1, 2)
